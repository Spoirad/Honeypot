...
from ftp_honeypot import run_ftp_honeypot

# ----- Argumentos -----

def start_thread(target, *args, name=None):
    #  lanzar target(...) en un hilo daemon y devolver el objeto Thread 
...
def main():
    ...
    # ejemplo ftp --> python honeypotController.py -a 0.0.0.0 --ftp --ftp-port 21
    ...
    parser.add_argument('-f', '--ftp', action="store_true", help="Arrancar honeypot FTP")
    
    parser.add_argument('--web-port', type=int, default=8080, help="Puerto para el honeypot web (-w)")
    parser.add_argument('--ftp-port', type=int, default=21, help="Puerto para el honeypot FTP (-f / --ftp)")
    ...
    try:
        ...
        if args.web:
            ...
            # lanzar honeypot web siguiendo la logica previa
        ...
        if args.ftp:
            print("Arrancando FTP honeypot...")
            # run_ftp_honeypot(address, port) no necesita usuario/contraseña, solo registra intentos
            t_ftp = start_thread(run_ftp_honeypot, args.address, args.ftp_port, name="FTP-Honeypot")
            threads['ftp'] = t_ftp
            print(f"FTP honeypot arrancado en hilo (escuchando en {args.address}:{args.ftp_port}).")
        ...
        print("\nControl console: escribe 'status', 'stop <ssh|web|ftp|all>' o 'exit'")
        while True:
